...
class CrudGeneratorTest extends TestCase
{
    ...
    private function makeModel(array $overrides = []): CrudGenerator
    {
        return CrudGenerator::factory()->make($overrides);
    }

    private function createModel(array $overrides = []): CrudGenerator
    {
        return CrudGenerator::factory()->create($overrides);
    }

    public function test_can_create_model(): void
    {
        $model = $this->makeModel();
        $this->assertInstanceOf(CrudGenerator::class, $model);
    }

    public function test_can_store_model(): void
    {
        $model = $this->createModel();
        $this->assertDatabaseHas('crudgenerators', ['id' => $model->id]);
    }
    ...
    public function test_can_delete_model(): void
    {
        ...
        $this->assertDatabaseMissing('crud_generators', [
            'id' => $model->id,
        ]);
    }
    ...
    {
        $model = $this->createModel();
        $found = CrudGenerator::find($model->id);
